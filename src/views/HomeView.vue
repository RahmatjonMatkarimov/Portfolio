...
<template>
  <div class="transition-colors duration-500 overflow-x-hidden">
    <!-- Progress Bar -->
    <div 
      class="fixed top-0 left-0 h-1 bg-gradient-to-r from-primary to-purple-600 z-[100] transition-all duration-300"
      :style="{ width: `${scrollProgress}%` }"
    ></div>

    <!-- Hero Section -->
    <section class="min-h-[90svh] flex items-center relative overflow-hidden bg-white dark:bg-[#020617] transition-colors duration-500 pt-12 md:pt-16">
      
      <!-- Background Effects -->
      <div class="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div class="absolute top-[-10%] right-[-5%] w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-primary/20 rounded-full blur-[80px] md:blur-[120px] animate-blob transition-all"></div>
        <div class="absolute bottom-[-10%] left-[-5%] w-[250px] md:w-[500px] h-[250px] md:h-[500px] bg-purple-500/10 rounded-full blur-[80px] md:blur-[120px] animate-blob animation-delay-4000 transition-all"></div>
        <div class="grain-overlay opacity-[0.03]"></div>
      </div>
      ...
      <div class="container mx-auto px-6 relative z-10">
        <div class="flex flex-col lg:flex-row items-center justify-between gap-10 md:gap-12">
          ...
          <div class="flex-1 text-center lg:text-left">
            ...
            <h1 class="font-display text-3xl sm:text-4xl md:text-5xl lg:text-5xl xl:text-6xl font-black mb-5 tracking-tight leading-[1.05] animate-fade-in-up">
              <span class="block text-slate-900 dark:text-white">{{ t.hero.title1 }}</span>
              <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-600 animate-text-shimmer bg-[length:200%_auto]">
                {{ t.hero.title2 }}
              </span>
            </h1>
            
            <p class="text-sm md:text-base text-slate-600 dark:text-slate-400 mb-6 max-w-2xl mx-auto lg:mx-0 leading-relaxed font-light animate-fade-in-up" style="animation-delay: 0.1s">
              {{ t.hero.subtitle }} <span class="font-bold text-slate-900 dark:text-white border-b-2 border-primary/20">{{ personalInfo.name }}</span>. 
              {{ personalInfo.shortBio[currentLanguage] }}
            </p>

            <div class="flex flex-col sm:flex-row items-center gap-3 justify-center lg:justify-start animate-fade-in-up" style="animation-delay: 0.2s">
              <router-link to="/projects" class="w-full sm:w-auto group relative px-6 py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-lg font-bold text-sm md:text-base overflow-hidden transition-all duration-300 hover:scale-[1.02] shadow-lg">
                <span class="relative z-10 flex items-center justify-center gap-2.5">
                  {{ t.hero.action1 }}
                  <ArrowRight class="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </span>
                <div class="absolute inset-0 bg-gradient-to-r from-primary to-purple-600 opacity-0 group-hover:opacity-20 transition-opacity duration-300"></div>
              </router-link>
              ...
              >
                ...
                <span class="relative z-10 flex items-center justify-center gap-2.5 text-slate-700 dark:text-slate-300 group-hover:text-primary transition-colors">
                  {{ t.hero.action2 }}
