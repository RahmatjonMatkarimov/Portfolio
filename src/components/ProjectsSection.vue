...
<template>
  <section id="projects" class="py-12 md:py-20 relative overflow-hidden transition-colors duration-500"
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none z-0">
       <div class="absolute top-0 left-1/4 w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-primary/5 rounded-full blur-[80px] md:blur-[120px] animate-blob mix-blend-screen opacity-50"></div>
    </div>

    <div class="container mx-auto px-6 relative z-10">
      <div class="flex flex-col md:flex-row justify-between items-center md:items-end mb-10 md:mb-16 gap-6 text-center md:text-left">
        <div class="max-w-3xl">
      ...
      <div class="grid gap-8 lg:grid-cols-2">
        <div v-for="(project, index) in projects.slice(0, limit || projects.length)" :key="index"
          ...
          <div class="absolute inset-0 rounded-[1.25rem] md:rounded-[2rem] bg-gradient-to-br transition-all duration-700 opacity-0 group-hover:opacity-5 shadow-2xl"
               :class="getGradient(index)"></div>
          
          <div class="relative z-10">
            <div class="flex justify-between items-start mb-5 md:mb-8">
              <div class="p-2.5 md:p-4 rounded-xl md:rounded-2xl bg-primary/10 text-primary transition-transform duration-500 group-hover:rotate-6 group-hover:scale-110">
            ...
            <h3 class="text-xl md:text-3xl font-black mb-2 md:mb-3 tracking-tight leading-tight" :class="isDark ? 'text-white' : 'text-slate-900'">
              {{ project.title[currentLanguage] }}
            </h3>

            <p class="mb-5 md:mb-8 text-sm md:text-base font-light leading-relaxed opacity-80" :class="isDark ? 'text-slate-400' : 'text-slate-600'">
      ...
      <div v-if="limit && projects.length > limit" class="mt-12 md:mt-20 text-center">
          <router-link to="/projects" class="inline-flex items-center gap-2.5 px-5 md:px-8 py-2.5 md:py-4 rounded-xl md:rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-xs md:text-sm transition-all duration-500 hover:scale-105 group shadow-xl">
              {{ t.projects.viewAll }} <ArrowUpRight class="w-4 h-4 md:w-5 md:h-5 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
          </router-link>
      </div>
    </div>
  </section>
