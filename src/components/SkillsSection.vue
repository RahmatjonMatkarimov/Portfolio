...
<template>
  <section id="skills" class="py-12 md:py-20 relative overflow-hidden transition-colors duration-500"
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none z-0">
      <div
        class="absolute top-[20%] right-[10%] w-[300px] md:w-[800px] h-[300px] md:h-[800px] bg-primary/5 rounded-full blur-[60px] md:blur-[150px] animate-blob mix-blend-screen opacity-50 will-change-transform">
      </div>
    </div>
    ...
    <div class="container mx-auto px-6 relative z-10">
      ...
      <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-7 lg:gap-8">
        <div v-for="(items, category) in skills" :key="category"
          ...
          <div
            class="absolute top-0 right-0 w-32 md:w-48 h-32 md:h-48 bg-primary/20 rounded-full blur-[60px] md:blur-[80px] transform translate-x-12 -translate-y-12 group-hover:scale-125 transition-transform duration-1000 opacity-0 group-hover:opacity-30">
          </div>

          <div class="relative z-10">
            <div
              class="p-2.5 md:p-4 rounded-xl md:rounded-2xl bg-primary/10 text-primary transition-transform duration-700 group-hover:rotate-6 group-hover:scale-110 w-fit mb-5 md:mb-8">
            ...
            <div class="flex flex-col gap-2.5">
              <div v-for="skill in items" :key="skill"
                ...
                <div
                  class="flex items-center justify-between group-hover/skill:translate-x-2 transition-transform duration-300">
                  <span
                    class="text-base md:text-xl font-bold tracking-tight opacity-70 group-hover/skill:opacity-100 transition-opacity"
                    :class="isDark ? 'text-white' : 'text-slate-900'">
                    {{ skill }}
                  </span>
                  <div
                    class="w-1.5 h-1.5 rounded-full bg-primary opacity-0 group-hover/skill:opacity-100 transition-opacity">
                  </div>
                </div>
              </div>
            </div>
